...
|-------------|----------|
| **Оценка моделей** | Скрипты `evaluate.py` и `evaluate_text_only.py` позволяют команде NLP VK быстро проводить бенчмаркинг-анализ различных VLM на русскоязычных датасетах GQA-ru и MMBench-ru |
